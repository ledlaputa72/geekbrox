...
const tierInfo = {
  ...
  uncommon: { name: '희귀', color: 'text-green-400', bgColor: 'from-green-700 to-green-800', borderColor: 'border-green-600' },
  rare: { name: '에픽', color: 'text-blue-400', bgColor: 'from-blue-700 to-blue-800', borderColor: 'border-blue-600' },
  epic: { name: '전설', color: 'text-purple-400', bgColor: 'from-purple-700 to-purple-800', borderColor: 'border-purple-600' },
  legendary: { name: '회귀', color: 'text-orange-400', bgColor: 'from-orange-700 to-orange-800', borderColor: 'border-orange-600' },
};
...
export default function EquipmentDetailModal({ equipment, onClose, onEquip, onUnequip, isEquipped = false }: EquipmentDetailModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center p-4 z-50">
      <div className="bg-[#f8edcd] rounded-xl border-2 border-gray-400 p-5 max-w-sm w-full shadow-2xl">
        {/* 상단: 아이콘 + 이름 + 등급 */}
        <div className="text-center mb-5 pb-4 border-b border-gray-300">
          <div className="inline-block mb-3 p-3 bg-gray-200 rounded-lg">
            <div className="text-5xl">{equipment.icon}</div>
          </div>
          <h3 className="text-xl font-bold mb-1 text-gray-800">{equipment.name}</h3>
          <div className="flex justify-center gap-2 text-sm">
            <span className={tier.color}>{tier.name}</span>
            <span className="text-gray-500">•</span>
            <span className="text-gray-600">{typeNames[equipment.type]}</span>
            <span className="text-gray-500">•</span>
            <span className="text-gray-600">LV {equipment.level}</span>
          </div>
        </div>

        {/* 기본 능력치 */}
        <div className="mb-4">
          <div className="text-sm font-bold text-gray-700 mb-2">기본 능력치</div>
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-gray-200 rounded p-2">
              <div className="text-xs text-gray-600">공격력</div>
              <div className="text-sm font-bold text-yellow-600">{details.stats.atk}</div>
            </div>
            <div className="bg-gray-200 rounded p-2">
              <div className="text-xs text-gray-600">방어력</div>
              <div className="text-sm font-bold text-blue-600">{details.stats.def}</div>
            </div>
          </div>
        </div>
        ...
        {/* 장비 스킬 */}
        <div className="mb-4">
          <div className="text-sm font-bold text-gray-700 mb-2">장비 스킬</div>
          <div className={`bg-gray-100 rounded p-3 border-l-2 ${skillColorClass}`}>
            <div className="text-xs font-bold mb-1 text-gray-800">{details.skills}</div>
        ...
        {details.options.length > 0 && (
          <div className="mb-4">
            <div className="text-sm font-bold text-gray-700 mb-2">추가 옵션</div>
            <div className="space-y-1">
              {details.options.map((option, idx) => (
                <div key={idx} className="text-xs text-gray-600">
                  <span className="text-green-600">+</span> {option}
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="mb-4 p-3 bg-gray-100 rounded border border-gray-300">
          <div className="flex justify-between items-center">
            <div className="text-xs text-gray-600">사용 가능 횟수</div>
        ...
        <div className="flex gap-2">
          ...
          {isEquipped ? (
            <>
              ...
              <button
                className="flex-1 bg-gradient-to-r from-purple-600 to-purple-500 hover:from-purple-500 hover:to-purple-400 py-2.5 rounded-lg font-bold text-sm transition text-white"
              >
                강화
              </button>
              <button
                onClick={onClose}
                className="flex-1 bg-gray-400 hover:bg-gray-300 py-2.5 rounded-lg font-bold text-sm transition text-gray-800"
              >
                닫기
              </button>
            </>
            ...
            <>
              <button
                onClick={() => {
                  if (onUnequip) onUnequip(equipment);
                  onClose();
                }}
                className="flex-1 bg-red-600 hover:bg-red-500 py-2.5 rounded-lg font-bold text-sm transition text-white"
              >
                장비해제
              </button>
              <button
