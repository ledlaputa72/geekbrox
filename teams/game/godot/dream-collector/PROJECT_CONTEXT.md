...
**3단계 우선순위**:
```
Level 1: Cursor (무료 로컬) → 최우선 사용
   ↓ 크레딧 소진
Level 2: Claude Code (무료 티어) → 백업
   ↓ 크레딧 소진
Level 3: Atlas (유료 API) → 최후 수단 + 전략적 작업
   ↓ 리셋 대기
Level 1 복귀
```

**목표**: 유료 API 최소화, 무료 리소스 최대 활용

**자세한 내용**: `CREDITS.md` 참고

### Cursor IDE (Level 1 - 최우선)
- **용도**: 모든 코드 편집, 버그 수정, 리팩터링
- **비용**: 무료 (로컬 모델)
- **한도**: 무제한
- **속도**: 매우 빠름
- **보고**: CHANGELOG + 텔레그램

### Claude Code (Level 2 - 백업)
- **용도**: 대화형 디버깅, 단계별 구현
- **비용**: 무료 (프리티어 API)
- **한도**: ~500 메시지/일
- **리셋**: 매일 자정 UTC (오전 9시 PST)
- **보고**: CHANGELOG + 텔레그램
...
- **용도**: 
  ...
  - 🎯 복잡한 아키텍처 결정
  - 🎯 Git/Notion 승인 및 실행
- **비용**: 유료 API (~$0.01-0.05/메시지)
- **한도**: $100/월 (조정 가능)
- **가용**: 24시간

### 사용 규칙
- **일반 작업 (80%)**: Cursor (Level 1)
- **복잡한 디버깅 (15%)**: Claude Code (Level 2)
- **전략/정리 (5%)**: Atlas (Level 3)
- **전환**: 크레딧 소진 시 다음 레벨로
