...
**작성일:** 2025-03  
**대상:** 클로드·텔레그램 공유용  
**범위:** 탐험 로그 제한, 리액션(!) 표시, 패링/회피/가드 버튼·규칙·피드백, 턴베이스 보스전 리액션 활성화, **메뉴얼/오토 방안 및 카드 디자인**
...
## 5. 패링/회피/가드 메뉴얼 vs 오토

### 메뉴얼 (수동)
- **모든 조작 가능.** 리액션 윈도우에서 패링/회피/가드 버튼 및 카드 탭으로 타이밍에 맞게 사용.
- 패링은 빨간 구간, 회피는 노랑+빨강, 가드는 전체 구간에서 성공.

### 오토 (Auto: ON)
| 행동 | 동작 |
|------|------|
| **패링** | **사용 불가.** 패링 태그 카드는 **✕ 표시 + 비활성화** (선택·사용 불가). 메뉴얼로 전환 시 정상화. |
| **회피** | **카드별 오토 성공 확률**로 판정. 해당 확률에 따라 성공/실패 처리. |
| **가드** | **100% 성공.** 가드 카드가 있으면 우선 사용. |

### 카드별 오토 회피 성공률 (예시)
- 꿈의 스텝 50%, 잔상 55%, 황혼의 도약 60%, 연막 50%, 반보 앞으로 45%, 완벽한 방어 50% (회피로 사용 시).

---

## 6. 카드 디자인 (레이아웃 원칙)

- **상단 30%:** 카드 그래픽 (일러스트/플레이스홀더).
- **하단 70%:** 카드 이름·종류·능력·**오토 시 확률(해당 시)**·업그레이드 상태 등.
- 회피(DODGE) 태그 카드는 하단 정보 영역에 **「오토 성공률 N%」** 표시.
- 오토 모드일 때 패링(PARRY) 태그 카드는 카드 중앙에 **✕** 오버레이 + 비활성화.
...
## 2.8 메뉴얼/오토 리액션 및 카드 UI

- **Card.gd:** `auto_dodge_success_rate: float = 0.5` 추가, `duplicate_card()`에서 복사.
- **CardDatabase.gd:** DODGE(및 PARRY+DODGE) 카드별 `auto_dodge_success_rate` 설정 (0.45~0.6).
- **TurnBasedAutoAI.decide_defense:** 패링 선택 제거. 가드 100% 우선 → 회피는 `card.auto_dodge_success_rate`로 randf() 판정.
- **CombatManagerTB:** 리액션 윈도우 오픈 후 오토 FULL이면 0.3초 뒤 `auto_ai.decide_defense()` 호출, 카드 있으면 `player_play_card(card)`.
- **CombatManagerATB:** `_try_auto_reaction()` 추가. 가드 우선(100%), 회피는 카드별 확률, 패링 미사용. 리액션 오픈 후 오토 FULL이면 0.3초 뒤 `_try_auto_reaction()` 호출.
- **CombatBottomUI:** `_card_to_dict`에 `tags`, `auto_dodge_success_rate` 포함. `_update_hand_ui`에서 오토 여부 판단 후 `card_item.set_auto_parry_disabled(is_auto)` 호출.
- **CardHandItem:** `set_auto_parry_disabled(is_auto)` — 오토이고 PARRY 태그면 ✕ 오버레이 + `_parry_disabled_by_auto`로 클릭/호버 무시, 회색 처리. 회피 카드 시 `_auto_rate_label`로 「오토 성공률 N%」 표시.
...
7. **메뉴얼:** 모든 리액션 조작 가능. **오토:** 패링 불가(패링 카드 ✕+비활성), 회피=카드별 확률, 가드=100%. 카드 하단에 회피 시 「오토 성공률 N%」 표시.
