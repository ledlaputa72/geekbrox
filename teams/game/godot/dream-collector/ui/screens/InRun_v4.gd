...
var _combat_monster_character_indices: Array[int] = []  # character_nodes 인덱스 → enemies 인덱스 매핑
...
func switch_to_combat(is_boss: bool = false):
	...
	# 몬스터 스폰 (enemies 수와 일치: ATB=3, TB=1)
	var spawn_count = 3 if not current_is_boss else 1
	print("[InRun_v4] Spawning monsters...")
	await _spawn_monsters(spawn_count)
...
func _start_atb_combat():
	...
	if atb_scene_res:
		...
		if manager:
			...
			# 몬스터 데이터 (Monster 노드 생성)
			var enemy_nodes = _create_test_monster_nodes(false)
			# 스타터 덱
...
func _start_tb_combat():
	...
	if tb_scene_res:
		...
		if manager:
			...
			var enemy_nodes = _create_test_monster_nodes(true)
			var deck = _get_starter_deck()
			manager.start_combat(p_data, enemy_nodes, deck)
			manager.combat_ended.connect(_on_new_combat_ended)
			manager.player_hp_changed.connect(_on_new_player_hp_changed)
...
func _on_new_enemy_hp_changed(enemy_idx: int, hp: int, _max_hp: int):
